...
class Vote(commands.Cog):
    ...
    @commands.command(
        brief="Starts a vote",
        description="Call this command with a vote string. Ex: [vote_command] Does pineapple belong on a pizza? ",
    )
    ...
    @commands.command(
        brief="Displays the results of a vote",
        description="If this command is called with any parameter, \
    ...
    async def voteresult(self, ctx, message_id=None):
        ...
        if message_id is not None:
            try:
                ...
                await ctx.send(
                    f"Usage: `{self.bot.command_prefix}voteresult [message_id (optional)]`"
                )
    ...
    @commands.command(
        brief="Removes a vote",
        description="Calling this command without any parameter, it will return the message ids of all the votes done by the user. Then the user can delete a specific vote.",
    )
    async def removevote(self, ctx, message_id=None):
            ...
            try:
                ...
                for i in self.data_vote:
                    if (
                        ...
                        and i["user_id"] == ctx.message.author.id
                    ):
        ...
        # Getting the message ids of all the votes done by the user calling this command
            ...
            for i in self.data_vote:
                ...
                if i["user_id"] == int(author_id) and i["channel_id"] == int(
                    ctx.channel.id
                ):
            ...
            embed_send = self.send_vote_message_ids(
                ctx.author.name, vote_strings, message_ids
            )
    ...
    async def utility_voteresultfull(self, ctx, msg):
        ...
        for i in message_reactions:
            ...
            if i.emoji == "👍":
                ...
                positive_users_temp = [
                    x.name for x in positive_users if x.name != self.bot.user.name
                ]
            ...
            # For negative reaction
                ...
                negative_users_temp = [
                    x.name for x in negative_users if x.name != self.bot.user.name
                ]
                negative_string = "\n".join(negative_users_temp)

        embed_send = self.send_vote_message_users(
            message_text, positive_string, negative_string
        )
        await ctx.send(embed=embed_send)
    ...
    async def utility_voteresult(self, ctx, msg):
        ...
        embed_send = self.send_vote_message(
            message_text, count_positive, count_negative
        )
        await ctx.send(embed=embed_send)
    ...
    def send_vote_message(self, vote_string, thumbsup_count, thumbsdown_count):
        ...
        embed = Embed(
            title=vote_string, colour=Color(int(hex(random.randint(1, 16581374)), 16))
        )
    ...
    def send_vote_message_ids(self, user_name, vote_strings, message_ids):
        # Creating an embed to send the result in a pretty manner with the users list
        embed = Embed(
            title=user_name, colour=Color(int(hex(random.randint(1, 16581374)), 16))
        )
